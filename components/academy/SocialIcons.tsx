...
export type SocialIconName = "facebook" | "x" | "linkedin" | "instagram" | "tiktok" | "youtube" | "whatsapp" | "reddit";
...
export default function SocialIcon({ name, className }: { name: SocialIconName; className?: string }) {
  switch (name) {
    ...
    case "reddit":
      return (
        <svg aria-hidden="true" viewBox="0 0 24 24" className={className} fill="currentColor">
          <circle cx="12" cy="14.2" r="6.3" fill="none" stroke="currentColor" strokeWidth="1.6" />
          <circle cx="9.2" cy="14" r="1.1" />
          <circle cx="14.8" cy="14" r="1.1" />
          <path d="M9 17.2c.8.6 1.9.9 3 .9s2.2-.3 3-.9" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
          <path d="M12 7.9V4.8M12 4.8l2.6.9M17.5 12a1.6 1.6 0 1 1 0-3.2" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
          <circle cx="14.9" cy="5.4" r="1.1" />
        </svg>
      );
  }
}
